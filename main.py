...
capture_opt = 'http://admin:admin@192.168.43.1:8081/'  # 摄像头参数，0，1为本机摄像头

# 继承wx库里面的Frame类来使用
class MainFrame(wx.Frame):
    def __init__(self):
        # 初始化窗体数据
        wx.Frame.__init__(self, None, -1, '人脸考勤系统', pos=(100, 100), size=(1337, 600))
        ...
        self.text9.SetForegroundColour('Red')
    ...
    def updateSumData(self):
        ...
        for list in os.listdir(picture_dir_path):
            if len(os.listdir(picture_dir_path + '/' + list)) >= 200:
                self.data_num += 1
        self.text10.SetLabel(u'当前已采集人脸数据的学生数：' + str(self.data_num))
        self.text10.Update()
    ...
    def recognitionFace(self):
        ...
        while self.recognition == False:
            ...
            if self.recognition and stu_name != '':
                ...
                self.Message = wx.MessageDialog(self, '亲爱的'+stu_name+'同学：打卡成功！', '成功',
                                                          wx.ICON_MASK)
    ...
    def startCollect_Event(self,event):
        ...
        if result == 1:
            ...
            if not os.path.exists(picture_dir_path+'/Stu_' + str(face_num)):  # 文件夹是否存在
                ...
                self.Message = wx.MessageDialog(self, '该学生的信息已录入！请重新输入！', '错误', wx.ICON_ERROR)
                self.Message.ShowModal()
                self.Message.Destroy()
        else:
        ...
        if flag == 1 :
            if result ==1:
                self.Message = wx.MessageDialog(self, '学生信息录入数据库成功！确认后开始采集人脸！', '成功', wx.ICON_ASTERISK)
            else:
                self.Message = wx.MessageDialog(self, '该学生信息已在数据库，但是人脸数据未成功采集！\n确认后将重新采集人脸数据！', '警告', wx.ICON_WARNING)
            self.Message.ShowModal()
            self.Message.Destroy()
            self.Hide()
            second = 3
...
class MyGrid(wx.grid.Grid):
    def __init__(self, parent,mySqlDao):
        ...
        self.CreateGrid(0, 4) #设置初始化时的行数和列数
        #为列定义初始化名字
...
class MySQLDao():
    ...
    def getCheckData(self):
        ...
        cur = con.cursor()
        # 编写查询的sql
        sql = 'select s.sno,s.sname,s.college,c.check_time from t_stu s,t_check c where s.id = c.uid order by c.check_time desc'
        # 执行sql
        try:
            cur.execute(sql)
            # 处理结果集
    ...
    '''
    '''
    def addCheckData(self,uid):
        flag = 1
        con = sqlite3.connect(database_file_path)
        # 创建游标对象
        cur = con.cursor()
        # 编写插入数据的sql
        sql = 'insert into t_check(uid,check_time) values(?,?)'
        dt = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        try:
            # 执行sql
            cur.execute(sql, (uid, dt))
            # 提交事务
            con.commit()
            return flag
        except Exception as e:
